...
def generate_dataset(n_samples=500000, fraud_rate=0.01):
    """
    Generate dataset with location, IP address, region, card number, transaction amount, and MCC.
...
def prepare_data(df):
    ...
    X = df[['location_ip_mismatch', 'is_card_valid', 'transaction_amount', 'mcc_amount_mismatch']].values
...
def train_and_evaluate_all_models(n_samples=1000, fraud_rate=0.01):
    ...
    features = ['location_ip_mismatch', 'is_card_valid', 'transaction_amount', 'mcc_amount_mismatch']
