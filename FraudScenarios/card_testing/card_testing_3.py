...
def generate_dataset(n_samples=500000, fraud_rate=0.01):
    """
    Generate dataset with billing address, IP address, region, card number, transaction amount, and MCC.
    Fraud is determined by:
    1. Invalid credit card numbers AND
    2. Transaction amount > $2000 AND
    3. MCC inconsistent with high transaction amount
    """
...
def prepare_data(df):
    ...
    X = df[['ip_region_mismatch', 'is_card_valid', 'transaction_amount', 'mcc_amount_mismatch']].values
...
def train_and_evaluate_all_models(n_samples=1000, fraud_rate=0.01):
    ...
    features = ['ip_region_mismatch', 'is_card_valid', 'transaction_amount', 'mcc_amount_mismatch']
