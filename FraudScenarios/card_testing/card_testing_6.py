...
def generate_dataset(n_samples=5000, fraud_rate=0.01):
    ...
    MCC, transaction type, device type, and browser information.
    ...
    for _ in range(n_fraud):
        ...
        # Generate device and browser information
        device, browser, is_compatible = generate_device_browser(ip_region, is_fraud=True)
...
def prepare_data(df):
    ...
    # Convert device type to one-hot encoding
    device_type_dummies = pd.get_dummies(df['device_type'], prefix='device')

    # Convert browser to one-hot encoding
