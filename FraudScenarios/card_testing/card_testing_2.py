...
def generate_dataset(n_samples=5000, fraud_rate=0.01):
    """
    Generate dataset with billing address, IP address, region, card number, and transaction amount.
    Fraud is determined by invalid credit card numbers AND transaction amount > $2000.
    """
    ...
    for _ in range(n_legitimate):
        ...
        # Generate random transaction amount (most transactions under $2000)
        amount = np.random.lognormal(mean=5.0, sigma=1.0)  # Centers around ~$150 with tail up to $2000
        amount = min(amount, 2000)  # Cap legitimate transactions at $2000
    ...
    # Generate fraudulent transactions (invalid card numbers AND high amounts)
    fraud_data = []
...
def prepare_data(df):
    ...
    X = df[['ip_region_mismatch', 'is_card_valid', 'transaction_amount']].values
...
def train_and_evaluate_all_models(n_samples=1000, fraud_rate=0.01):
    ...
    features = ['ip_region_mismatch', 'is_card_valid', 'transaction_amount']
