# Import all necessary libraries from card_testing_10.py
import pandas as pd
import numpy as np
import torch
...
# Constants from card_testing_10.py
MCC_RANGES = {
    'retail': (10, 100),
    'travel': (100, 1000),
...
# Helper functions remain the same as in card_testing_10.py
...
def generate_dataset(n_samples, fraud_rate=0.01):
    ...
    for _ in range(n_legitimate):
        ...
        # For legitimate transactions, decline some randomly and if amount exceeds credit limit
        transaction_declined = (amount > credit_limit) or (np.random.random() < 0.05)
        ...
        data.append({
            ...
            'is_fraud': False,
            'transactions_past_15min': 0  # Will be updated later
        })

    # Generate fraudulent transactions with clustered timestamps
    fraud_clusters = n_fraud // 15 + (1 if n_fraud % 15 > 0 else 0)

    for cluster in range(fraud_clusters):
        # Generate base timestamp for this cluster
        base_timestamp = start_date + (end_date - start_date) * np.random.random()

        # Generate invalid card number for this cluster
        card_number = generate_card_number(is_valid=False)

        # Generate other consistent details for this cluster
        region = np.random.choice(regions)
        mcc = np.random.choice(mccs)
        credit_limit, card_type = generate_credit_limit()

        # Ensure device-browser compatibility for fraud
        device, browser, is_compatible = generate_device_browser(region, is_fraud=True)
        while not is_compatible:
            device, browser, is_compatible = generate_device_browser(region, is_fraud=True)

        # Generate location from different region
        other_regions = [r for r in regions if r != region]
        other_region = np.random.choice(other_regions)
        location = np.random.choice(REGION_LOCATIONS[other_region])

        # Generate 15 transactions for this cluster
        for i in range(15):
            # Amount between 200 and 2000, exceeding credit limit
            amount = np.random.uniform(
                max(200, credit_limit + 100),
                min(2000, credit_limit * 1.5)
            )
            ...
            transaction = {
                ...
                'transaction_declined': True,  # Always declined for fraud
                'is_fraud': True,
                'transactions_past_15min': min(i, 10)  # Will be updated later
            }
            data.append(transaction)

            # Break if we've generated enough fraud transactions
            if len([d for d in data if d['is_fraud']]) >= n_fraud:
                break
    ...
    print(f"Declined Transactions: {df['transaction_declined'].sum()}")
...
def prepare_data(df):
    ...
    binary_features = ['is_card_valid', 'mcc_amount_mismatch', 'location_ip_mismatch',
                      'device_browser_compatible', 'exceeds_credit_limit', 'transaction_declined']
