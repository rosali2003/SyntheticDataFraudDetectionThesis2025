# Add imports from card_testing_7.py
import pandas as pd
import numpy as np
import torch
...
# Import all constant dictionaries from card_testing_7.py
MCC_RANGES = {
    # Retail stores
...
# Import all other dictionaries and classes from card_testing_7.py
# ... [Keep all other constant dictionaries and helper functions from card_testing_7.py] ...
...
def calculate_time_since_last_transaction(transactions_df):
    """
    Calculate the time since last transaction for each transaction
    Returns time difference in minutes
    """
    # Sort transactions by timestamp
    sorted_df = transactions_df.sort_values('timestamp')

    # Calculate time difference between consecutive transactions
    sorted_df['time_since_last'] = sorted_df['timestamp'].diff().dt.total_seconds() / 60

    # Fill NaN (first transaction) with a large value
    sorted_df['time_since_last'] = sorted_df['time_since_last'].fillna(1440)  # 24 hours in minutes
...
def generate_dataset(n_samples=5000, fraud_rate=0.01):
    ...
    MCC, transaction type, device type, browser information, authentication method,
    ...
    # Generate base timestamp (24 hours ago)
    base_timestamp = datetime.now() - timedelta(days=1)

    # Generate legitimate transactions
    legitimate_data = []
    for _ in range(n_legitimate):
        # Generate timestamp with random minutes added
    ...
    # Combine and create initial DataFrame
    df = pd.DataFrame(legitimate_data + fraud_data)

    # Calculate time since last transaction
    df = calculate_time_since_last_transaction(df)
...
def prepare_data(df):
    ...
    numerical_features = ['transaction_amount', 'time_since_last']
...
def train_and_evaluate_all_models(n_samples=1000, fraud_rate=0.01):
    ...
    print("Average time between transactions (minutes):", dataset_df['time_since_last'].mean())
    print("Median time between transactions (minutes):", dataset_df['time_since_last'].median())
    print("Max time between transactions (minutes):", dataset_df['time_since_last'].max())
    print("Min time between transactions (minutes):", dataset_df['time_since_last'].min())
