...
def generate_dataset(n_samples=5000, fraud_rate=0.01):
    """
    Generate dataset with location, IP address, region, card number, transaction amount, MCC, and transaction type.
    Fraud is determined by:
    1. Invalid credit card numbers AND
    2. Transaction amount > $2000 AND
    3. MCC inconsistent with high transaction amount AND
    4. Location does not match IP region
    """
    ...
    for _ in range(n_legitimate):
        ...
        # Select random MCC and generate appropriate amount
        mcc = np.random.choice(list(MCC_RANGES.keys()))
        min_amount, max_amount = MCC_RANGES[mcc]

        # Generate amount within typical range for MCC
        amount = np.random.uniform(min_amount, max_amount)

        # Generate appropriate transaction type
    ...
    for _ in range(n_fraud):
        ...
        # Select MCC with typically low transaction amounts
        low_amount_mccs = [mcc for mcc, (_, max_amt) in MCC_RANGES.items() if max_amt < 1000]
        mcc = np.random.choice(low_amount_mccs)

        # Generate high transaction amount (> $2000)
        amount = np.random.uniform(2000, 10000)

        # Generate transaction type (fraudulent transactions tend to be credit)
...
def prepare_data(df):
    ...
    df['high_amount'] = (df['transaction_amount'] > 2000).astype(int)
    ...
    feature_columns = [
        'location_ip_mismatch', 'is_card_valid', 'transaction_amount',
        'mcc_amount_mismatch', 'trans_type_credit', 'trans_type_debit',
        'trans_type_cash'
    ]
...
def train_and_evaluate_all_models(n_samples=1000, fraud_rate=0.01):
    ...
    print("High Amount Transactions (>$2000):", sum(dataset_df['transaction_amount'] > 2000))
    print("Transaction Type Distribution:")
    print(dataset_df['transaction_type'].value_counts())
    print("MCC-Amount Mismatches:", sum(dataset_df.apply(
        lambda row: is_amount_suspicious_for_mcc(row['transaction_amount'], row['mcc']),
        axis=1
    )))
