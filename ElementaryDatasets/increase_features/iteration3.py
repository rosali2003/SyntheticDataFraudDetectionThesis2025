...
def generate_dataset(n_samples=1000, fraud_rate=0.01):
    """
    Generate dataset with IP address, billing region, shipping region, and blacklist features.
    ...
    # Generate legitimate transactions (matching regions, not blacklisted)
    legitimate_data = []
    for _ in range(n_legitimate):
        ...
        legitimate_data.append({
            ...
            'is_blacklisted': 0,  # Not blacklisted for legitimate transactions
            'is_fraud': 0
        })
    ...
    for _ in range(n_fraud):
        ...
        # 1. All mismatches + blacklisted
        # 2. Region mismatches + blacklisted
        # 3. IP-billing mismatch + shipping mismatch + blacklisted
        fraud_type = np.random.choice(['all', 'regions', 'mismatch'])

        if fraud_type == 'all':
            # All regions different and blacklisted
        ...
        elif fraud_type == 'regions':
            # Different regions but same IP-billing, blacklisted
            ...
            fraud_data.append({
                'ip_address': ip_address,
                'billing_region': region1,
                'shipping_region': region2,
                'is_blacklisted': 1,
                'is_fraud': 1
            })
        else:  # mismatch
            # IP-billing mismatch + shipping mismatch
            region1, region2 = np.random.choice(list(regions.keys()), size=2, replace=False)
            ip_prefix = np.random.choice(regions[region1])
            ip_suffix = np.random.randint(0, 256)
            ip_address = f"{ip_prefix}.{ip_suffix}.0.0"
            shipping_region = np.random.choice([r for r in regions.keys() if r != region2])
            fraud_data.append({
                'ip_address': ip_address,
...
def prepare_data(df, n_samples=10000, fraud_rate=0.01):
    ...
    X = df[['ip_region_mismatch', 'shipping_region_mismatch', 'is_blacklisted']].values
...
def train_and_evaluate_all_models(n_samples=1000, fraud_rate=0.01):
    ...
    print("Blacklisted IPs:", sum(df['is_blacklisted']))
    ...
    features = ['ip_region_mismatch', 'shipping_region_mismatch', 'is_blacklisted']
...
if __name__ == "__main__":
    all_results = {}
    all_results[0.01] = train_and_evaluate_all_models(fraud_rate=0.01)

    # Save results to CSV
    for rate, df in all_results.items():
        df.to_csv(f'model_comparison_fraud_rate_{rate}_features_3.csv')
